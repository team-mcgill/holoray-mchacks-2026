...
class Mlp(nn.Module):
    ...
    def __init__(
        ...
        drop=0.0,
        use_conv=False,
        use_approx_gelu=False,  # Previous
    ):
        super().__init__()
        ...
        self.drop1 = nn.Dropout(float(drop_probs[0]))
        self.norm = (
            norm_layer(hidden_features) if norm_layer is not None else nn.Identity()
        )
        self.fc2 = linear_layer(hidden_features, out_features, bias=bias[1])
        self.drop2 = nn.Dropout(float(drop_probs[1]))
...
class BasicEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        scale_H = H // self.stride
        scale_W = W // self.stride

        a = F.interpolate(a, (scale_H, scale_W), mode="bilinear", align_corners=True)
        b = F.interpolate(b, (scale_H, scale_W), mode="bilinear", align_corners=True)
        c = F.interpolate(c, (scale_H, scale_W), mode="bilinear", align_corners=True)
        d = F.interpolate(d, (scale_H, scale_W), mode="bilinear", align_corners=True)
...
class Attention(nn.Module):
    ...
    def forward(
        self, x, context: Optional[Tensor] = None, attn_bias: Optional[Tensor] = None
    ):
        B, N1, C = x.shape
        h = self.heads
...
class EfficientUpdateFormer(nn.Module):
    ...
    def forward(
        ...
        virtual2point_mask: Optional[torch.Tensor] = None
        point2virtual_mask: Optional[torch.Tensor] = None

        if mask is not None:
            time_mask = mask.permute(0, 2, 1)
            virtual_mask = torch.ones(
                B, self.num_virtual_tracks, T, device=mask.device
            ).to(torch.bool)
            time_mask = torch.cat([time_mask, virtual_mask], dim=1)
            _, time_mask_N, _ = time_mask.shape
            time_mask = time_mask.view(B * time_mask_N, T)

            point_mask = mask.clone().reshape(B * T, -1)
            virtual_mask = virtual_mask.permute(0, 2, 1).reshape(B * T, -1)

            virtual2point_mask = point_mask[:, None] * virtual_mask[:, :, None]
            point2virtual_mask = virtual_mask[:, None] * point_mask[:, :, None]

            # Fix memory stride == 2 issue for the masks
            time_mask = time_mask.contiguous()
            virtual2point_mask = virtual2point_mask.contiguous()
            point2virtual_mask = point2virtual_mask.contiguous()
        ...
        for (
            time_block,
            space_virtual2point_block,
            space_virtual_block,
            space_point2virtual_block,
        ) in zip(
            self.time_blocks,
            ...
            time_tokens = time_block(time_tokens, mask=time_mask)
            ...
            virtual_tokens = space_virtual2point_block(
                virtual_tokens, point_tokens, mask=virtual2point_mask
            )

            virtual_tokens = space_virtual_block(virtual_tokens, mask=None)

            point_tokens = space_point2virtual_block(
                point_tokens, virtual_tokens, mask=point2virtual_mask
            )
...
class CrossAttnBlock(nn.Module):
    ...
    def forward(self, x, context, mask: Optional[torch.Tensor] = None):
        if mask is not None:
            assert (
                mask.shape[1] == x.shape[1]
            ), "There is something wrong with the mask shape."
            mask = mask[:, None, ...].expand(-1, self.cross_attn.heads, -1, -1)
            max_neg_value = -1e9
            attn_bias = (~mask) * max_neg_value
        else:
            attn_bias = None
