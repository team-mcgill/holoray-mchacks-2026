...
#     coords_A = torch_get_featuremap_coords(
#         (flow_shape["H"], flow_shape["W"]), device=flow_AB.device, keep_shape=True
#     )
...
#     else:
#         coords_B = (
#             einops.rearrange(coords_A, "N H W xy -> N xy H W", xy=2) + coords_B_delta
#         )
...
#     flat_coords = norm_coords.view(
#         coords.shape[0], -1, coords.shape[-1]
#     )  # flatten all the outshape
#     grid_coords = einops.rearrange(
#         flat_coords, "batch N xy -> batch 1 N xy", xy=2
#     )  # simulate HxW coord grid (but only 1xN)
...
class FlowInterpolator(object):
    def __init__(self, flow, additional_data=None):
        ...
        self.interp = RegularGridInterpolator(
            (flow_grid_ys, flow_grid_xs), data, bounds_error=False, fill_value=np.nan
        )
    ...
    def __call__(self, positions, method="linear"):
        ...
        """
        return self.interp(
            positions[:, ::-1], method=method
        )  # the scipy interpolator wants y, x coordinates
...
#     flow_grid_ys = np.arange(H)
#     flow_grid_xs = np.arange(W)
#     interp_flow = RegularGridInterpolator(
#         (flow_grid_ys, flow_grid_xs), flow, bounds_error=False, fill_value=np.nan
#     )
#     new_flow = interp_flow(current_positions, method="linear")

#     if occlusion_mask is not None:
#         interp_occl = RegularGridInterpolator(
#             (flow_grid_ys, flow_grid_xs),
#             occlusion_mask,
#             bounds_error=False,
#             fill_value=1,
#         )
...
#     """
#     sampled_flow = bilinear_interpolate_torch(flow_AB, coords_A[0, :], coords_A[1, :])
...
#     grid_flat = grid_flat.index_put(
#         indices=[flat_indices], values=flat_data, accumulate=True
#     )

#     counts_flat = torch.zeros((H * W, 1), dtype=flat_count.dtype, device=device)
#     counts_flat = counts_flat.index_put(
#         indices=[flat_indices], values=flat_count, accumulate=True
#     )
#     # data_a = data[:, y0, x0]
#     # data_b = data[:, y1, x0]
#     # data_c = data[:, y0, x1]
...
#         einops.reduce(torch.square(flow_ABA), "xy H W -> H W", xy=2, reduction="sum")
#     )
