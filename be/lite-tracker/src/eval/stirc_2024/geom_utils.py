...
class Bbox:
    ...
    def with_margins(self, margin_fraction):
        return Bbox.from_xyxy(
            (
                self.tl_x - int(margin_fraction * self.w),
    ...
    def crop_image(self, img):
        ...
        cropped = img[rounded.tl_y : rounded.br_y, rounded.tl_x : rounded.br_x, ...]
        return cropped
    ...
    def sample_img(self, img, target_box, interpolation=cv2.INTER_NEAREST):
        ...
        sample = cv2.warpPerspective(
            img, H, (target_box.w, target_box.h), flags=interpolation
        )
        return sample
...
def max_fitting_bbox(src_bbox, target_bbox):
    ...
    return Bbox(
        target_bbox.tl_x, target_bbox.tl_y, src_bbox.w * scale, src_bbox.h * scale
    )
...
def torch_H_proj(H, points):
    ...
    homo_points = torch.cat(
        (points, torch.ones((1, N_points), dtype=torch.float64, device=points.device)),
        dim=0,
    )
...
def in_bounds(x, lb, ub, axis=None):
    return np.logical_and(np.all(x >= lb, axis=axis), np.all(x < ub, axis=axis))
...
def cv_crop_bbox(img, bbox_xywh):
    ...
    cropped = cv2.warpPerspective(img, H, (bbox.w, bbox.h), flags=cv2.INTER_NEAREST)
    return cropped
...
def crop_bbox(img, src_bbox, out_H, out_W, with_padding=True):
    ...
    if with_padding:
        ...
        src_bbox_roi = Bbox.from_xyxy(
            (
                pad_left + src_bbox.tl_x,
...
class HCoordSystemGraph:
    ...
    def get(self, src_name, dst_name):
        ...
        self.plot(plot_file)
        raise RuntimeError(
            f"No known transformation from {src_name} to {dst_name}. Check {plot_file}"
        )
...
def H_interpolate(H_a, H_b, t):
    res = scipy.linalg.expm(
        (1 - t) * scipy.linalg.logm(H_a) + t * scipy.linalg.logm(H_b)
    )
    assert np.allclose(np.imag(res), 0)
    return np.real(res)


def torch_get_featuremap_coords(feature_map, device=None, keep_shape=False):
...
def find_TRS(left_coords, right_coords):
    ...
    A = einops.rearrange(
        A, "N (two four) -> 1 (N two) four", two=2, four=4
    )  # add batch
...
def flow_to_TRS(flow, mask=None):
    ...
    flow_shape = einops.parse_shape(flow, "H W xy")
    device = flow.device
    grid_left_img = torch_get_featuremap_coords(
        (flow_shape["H"], flow_shape["W"]), device=device, keep_shape=True
    )
    grid_right_img = einops.rearrange(grid_left_img, "xy H W -> H W xy", xy=2) + flow

    left_coords = einops.rearrange(grid_left_img, "xy H W -> (H W) xy", xy=2).to(
        torch.float32
    )
    right_coords = einops.rearrange(grid_right_img, "H W xy -> (H W) xy", xy=2).to(
        torch.float32
    )
...
def Affine_to_flow(A, left_coords, flow_shape):
    to_proj = torch.cat(
        (
            einops.rearrange(left_coords, "N xy -> xy N", xy=2),
            torch.ones(
                (1, left_coords.shape[0]),
                dtype=left_coords.dtype,
                device=left_coords.device,
            ),
        ),
        dim=0,
    )
    TRS_right_coords = einops.rearrange(torch.matmul(A, to_proj), "xy N -> N xy", xy=2)
    TRS_flow = einops.rearrange(
        TRS_right_coords - left_coords, "(H W) xy -> H W xy", **flow_shape
    )
    return TRS_flow
...
def get_H_scaling(H_a2b, pts_in_a):
    ...
    A_a2b, _ = cv2.estimateAffine2D(
        einops.rearrange(pts_in_a, "xy N -> N 1 xy", xy=2),
