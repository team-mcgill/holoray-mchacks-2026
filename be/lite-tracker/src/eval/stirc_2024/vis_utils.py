...
def cv2_hatch(canvas, mask, color=(0, 0, 0), alpha=1, **kwargs):
    ...
    hatch_mask = np.logical_and(mask, hatching > 0)
...
def cv2_colorbar(img, vmin, vmax, cmap=None):
    ...
    cbar = (255 * cmap(norm(cbar))[..., [2, 1, 0]]).astype(
        np.uint8
    )  # RGBA to opencv BGR
    ...
    cv2.putText(
        vis, text_min, text_min_bl, font, size, (255, 255, 255), thickness, cv2.LINE_AA
    )

    text_max = "{:.2f}".format(vmax)
    text_max_size, text_max_baseline = cv2.getTextSize(text_max, font, size, thickness)
    text_max_bl = (
        img.shape[1] - (text_margin + text_max_size[0]),
        img.shape[0] - (text_margin + text_max_baseline + separator_sz),
    )
    cv2.putText(
        vis, text_max, text_max_bl, font, size, (255, 255, 255), thickness, cv2.LINE_AA
    )
...
def plt_hatch(mask, ax):
    ...
    ax.contourf(mask, 1, hatches=["", "//"], alpha=0.0)
...
def cv2_colormap(
    img, cmap=None, vmin=None, vmax=None, do_colorbar=False, hatch_params=None
):
...
def vis_alignment(
    init_img, current_img, H_cur2init, init_xywh, margin=0, show_frames=True
):
    ...
    crop_bbox = gu.Bbox.from_xyxy(
        (
            init_bbox.tl_x - int(margin * init_bbox.w),
    ...
    template_crop_H = gu.H_bbox2bbox(
        crop_bbox,
    ...
    current_warped = cv2.warpPerspective(
        current_img, H_cur2init, (init_img.shape[1], init_img.shape[0])
    )
    ...
    current_in_template = draw_box_with_margins(
        current_in_template, int_in_crop_init_bbox, radius=5
    )
    alignment_vis = draw_box_with_margins(
        alignment_vis, int_in_crop_init_bbox, radius=5
    )

    if show_frames:
        alignment_vis = np.concatenate(
            (template, current_in_template, alignment_vis), axis=1
        )
...
def draw_box_with_margins(canvas, bbox, radius, color=(0, 0, 255)):
    vis = canvas.copy()
    cv2.circle(vis, (bbox.tl_x, bbox.tl_y), radius, color)
    cv2.circle(vis, (bbox.br_x, bbox.tl_y), radius, color)
    cv2.circle(vis, (bbox.br_x, bbox.br_y), radius, color)
    cv2.circle(vis, (bbox.tl_x, bbox.br_y), radius, color)
    return vis
...
def draw_text(
    ...
    while True:
        text_size, text_baseline = cv2.getTextSize(
            "".join(texts), font, size, thickness
        )
    ...
    if pos == "bl":
        ...
        text_bl = (canvas.shape[1] - (text_margin + text_W), text_margin + text_H)
    elif pos == "tl":
        text_bl = (text_margin, text_margin + text_H)
    elif pos == "br":
        text_bl = (
            canvas.shape[1] - (text_margin + text_W),
            canvas.shape[0] - (text_margin + text_baseline),
        )
    elif pos == "center":
        text_bl = (
            canvas.shape[1] // 2 - text_W // 2,
    ...
    for text_i, text in enumerate(texts):
        ...
        canvas = cv2.putText(canvas, text, bl, font, size, c, thickness, cv2.LINE_AA)
        bl = (bl[0] + cv2.getTextSize(text, font, size, thickness)[0][0], bl[1])
    return canvas


def draw_text_multiline_centered(
    img,
    texts,
    center_x,
    center_y,
    ...
    total_height = sum(size[0][1] for size in text_sizes) + y_gap * (
        len(text_sizes) - 1
    )
    ...
    for text, x_shift, text_size in reversed(list(zip(texts, x_shifts, text_sizes))):
        bottom_left = (
            center_x - max_width // 2 + x_shift,
            center_y + total_height // 2 - y_shift,
        )
        y_shift += text_size[0][1] + y_gap
        canvas = cv2.putText(
            canvas, text, bottom_left, font, size, color, thickness, cv2.LINE_AA
        )
    return canvas


def draw_text_multiline_centered_PIL(
    img,
    texts,
    center_x,
    center_y,
    ...
    draw.text(
        ...
        anchor="mm",
        align="center",
        stroke_width=thickness + extra_thickness,
        spacing=4,
    )
    draw.text(
        (center_x, center_y),
...
def draw_corners(canvas, corners, color, thickness=2, with_cross=True, with_TL=False):
    ...
    if with_TL:
        vis = cv2.circle(
            vis,
            tuple(pts[0, 0, :].astype(np.int32).tolist()),
            radius=2 * thickness,
            color=color,
            thickness=-1,
        )
...
def circle(img, center, radius, color, thickness=None, lineType=None, shift=4):
    ...
    if shift is not None:
        multiplier = 2**shift
        center = tuple(
            np.round(multiplier * np.array(center)).astype(np.int32).tolist()
        )
...
def blend_mask(
    ...
    if contours:
        cnt = compatible_contours(
            mask.astype(np.uint8) * 255, retrieval_mode=cv2.RETR_LIST
        )
...
def blend_with_alpha(img1, img2, img1_alpha):
    ...
    result = img1.astype(np.float32) * img1_alpha + img2.astype(np.float32) * (
        1 - img1_alpha
    )
...
def blend_with_alpha_premult(img1_premult, img2, img1_alpha):
    ...
    result = img1_premult.astype(np.float32) + img2.astype(np.float32) * (
        1 - img1_alpha
    )
    result = result.clip(0, 255).astype(np.uint8)
    return result
...
def compatible_contours(thresh, retrieval_mode=cv2.RETR_EXTERNAL):
    if is_cv2():
        (contours, _) = cv2.findContours(
            thresh, retrieval_mode, cv2.CHAIN_APPROX_SIMPLE
        )
    elif is_cv3():
...
def vis_flow_dots(
    flow, src_img, dst_img, grid_sz=30, dot_radius=3, cmap="viridis", mask=None
):
    H, W = flow.shape[:2]
    ...
    for i in range(pts_src.shape[1]):
        ...
        circle(src_vis, a, radius=dot_radius, thickness=-1, color=color)
        circle(dst_vis, b, radius=dot_radius, thickness=-1, color=color)
...
def vis_flow(
    ...
    for i in range(mask.size):
        ...
        if mask[i]:
            if (
                (occl is not None)
                and (occlusion_color is None)
                and (occl[np.unravel_index(i, occl.shape)] >= occl_thr)
            ):
                continue
            a = pts_src[:, i]
            b = pts_dst[:, i]
...
def checkerboard(h, w, c0, c1, blocksize):
    ...
    tile = einops.repeat(
        np.array([[c0, c1], [c1, c0]]),
        "mini_H mini_W ... -> (mini_H block_H) (mini_W block_W) ...",
        block_H=blocksize,
        block_W=blocksize,
    )
    ...
    grid = einops.repeat(
        tile,
        "tile_H tile_W ... -> (rows tile_H) (cols tile_W) ...",
        rows=rows,
        cols=cols,
    )
    grid = grid[:h, :w, ...]
...
def color_checkerboard(h, w, blocksize):
    ...
    colored = np.concatenate(
        (colored, 255 * einops.rearrange(bool_checkerboard, "H W -> H W 1")), axis=2
    )
    return colored
...
class FlowGUI(object):
    def __init__(self, left_img, right_img, flow_left_to_right, mask=None):
        ...
        if flow_left_to_right.shape[0] == 2:
            flow_left_to_right = einops.rearrange(
                flow_left_to_right, "xy H W -> H W xy", xy=2
            )
    ...
    def draw(self):
        ...
        while True:
            ...
            if c == ord("q"):
                ...
                left_img, right_img = self.current_images()
                left_vis, right_vis = vis_flow_align_new(
                    self.flow_left_to_right,
                    left_img,
                    right_img,
                    grid_sz=30,
                    occl=np.logical_not(self.mask),
                    occl_thr=0.5,
                )
    ...
    def handler(self, event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            radius = 6
            left_vis = cv2.circle(
                left_vis, (x, y), radius=radius, color=color, thickness=-1
            )
            right_vis = circle(
                right_vis, (right_x, right_y), radius=radius, color=color, thickness=-1
            )
...
def zoomins(img, center_coord, zooms, win_size, cross=True, cross_color=(0, 0, 255)):
    results = []
    for zoom in zooms:
        sample_bbox = gu.Bbox.from_cxcywh(
            (center_coord[0], center_coord[1], win_size / zoom, win_size / zoom)
        )
...
def resize(src, dsize, fx=None, fy=None, interpolation=None, align_corners=False):
    ...
    A = np.array([[fx, 0, 0], [0, fy, 0]], dtype=np.float32)
